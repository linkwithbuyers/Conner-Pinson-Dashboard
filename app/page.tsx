...
function NotesEditor({ record, noteOverride, onSaveNote }: { record: LeadRecord; noteOverride?: string; onSaveNote: (record: LeadRecord, value: string) => void }) {
...
const dirty = draft !== effectiveNotes;
return (
...
rows={3}
style={{ width: "100%", marginBottom: "6px", padding: "8px", fontFamily: "inherit", fontSize: "inherit" }}
/>
{dirty ? (
<div style={{ display: "flex", gap: "8px", marginBottom: "8px" }}>
<button className="card-toggle" onClick={() => onSaveNote(record, draft)}>Save Note</button>
<button className="card-toggle" onClick={() => setDraft(effectiveNotes)}>Cancel</button>
</div>
) : null}
</div>
);
}
function LeadCard({ record, archived, pinned, noteOverride, onSaveNote, onViewConversation, onArchive, onPin }: { record: LeadRecord; archived?: boolean; pinned?: boolean; noteOverride?: string; onSaveNote: (record: LeadRecord, value: string) => void; onViewConversation: (record: LeadRecord) => void; onArchive: (record: LeadRecord) => void; onPin: (record: LeadRecord) => void }) {
return (
<article className="lead-card">
<h3>{record.fullName}</h3>
<p className="lead-subtitle">{[record.title, record.company].filter(Boolean).join(" at ") || "Profile details unavailable"}</p>
<p><ContactValue href={emailHref(record)} text={record.email} fallback="Email unavailable" /></p>
<p><ContactValue href={phoneHref(record)} text={formatPhone(record.phone)} fallback="Phone unavailable" /></p>
{record.location ? <p>{simplifyLocation(record.location)}</p> : null}
<WatchedIndicator date={record.watchedAt} complete={record.hasWatched} />
<p>Video sent{record.videoSent ? `: ${formatDateOnly(record.videoSent)}` : ""}</p>
{record.kind === "reply-before-video" ? <p className="warning-note">Stop video manually in the Sheet if needed.</p> : null}
{record.sourceIncomplete ? <p className="warning-note">Some source details are incomplete.</p> : null}
<NotesEditor record={record} noteOverride={noteOverride} onSaveNote={onSaveNote} />
<div className="card-actions">
<button className="card-toggle" onClick={() => onArchive(record)}>{archived ? "Restore" : "Archive"}</button>
<button className="card-toggle" onClick={() => onPin(record)}>{pinned ? "Unpin" : "Pin"}</button>
<button className="card-toggle" onClick={() => onViewConversation(record)}>Initial Outreach</button>
{record.profileUrl ? <a className="linkedin-link" href={record.profileUrl} target="_blank" rel="noreferrer">Open LinkedIn</a> : <span>LinkedIn unavailable</span>}
</div>
</article>
);
}
